...
The type may also be nested in other types, such as STL containers and other
user-defined types.
...
struct Foo {
  ...
  std::uint8_t size;
  NOP_STRUCTURE(Foo, (data, size)); // Members are grouped with parenthesis.
};
...
removing fields, and handle data generated by both older and newer versions of
the same table type.
...
using StringsProtocol = std::vector<int>;
